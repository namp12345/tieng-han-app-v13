...
/* ============================================================
   v14 UI PATCHES — Additive only. No logic changed.
   ============================================================ */

// 1. Patch updateTopProgress to also update percentage + streak display
const _origUpdateTopProgress = updateTopProgress;
function updateTopProgress() {
  _origUpdateTopProgress();
  const all = Object.values(state.progressById);
  const completed = all.filter(x => x.isCompleted).length;
...
// 2. Patch renderSessionPanel for new design with progress bar
const _origRenderSessionPanel = renderSessionPanel;
function renderSessionPanel() {
  const s = currentSessionInfo();
  const pct = s.total > 0 ? Math.round((s.done / Math.max(3, s.total)) * 100) : 0;
  refs.sessionPanel.innerHTML = `
    <article class="session-card">
      <div class="session-head">
        <span class="session-title">📅 Phiên học hôm nay</span>
        <span class="session-badge">${s.time}</span>
      </div>
      <div class="session-progress-bar-wrap">
        <div class="session-progress-bar-fill" style="width:${pct}%"></div>
      </div>
      <p class="session-meta">Đã học: <b>${s.done}/${Math.max(3, s.total)}</b> · Phiên kế tiếp: <b>${s.nextUnlocked ? '✅ Đã mở' : '🔒 Đang khóa'}</b></p>
    </article>
  `;
}

// 3. Patch FlashcardSentence to use new completion-btn style
const _origFlashcardSentence = FlashcardSentence;
function FlashcardSentence(s) {
  const node = _origFlashcardSentence(s);
  const p = state.progressById[s.id];
  // Update completion button style
  const completionBtn = node.querySelector('[data-action="complete"]');
  const completionLabel = node.querySelector('[data-completion-label]');
  const completionIcon = node.querySelector('[data-completion-icon]');
  if (completionBtn && completionLabel) {
    if (p.isCompleted) {
      completionBtn.classList.add('is-done');
      if (completionIcon) completionIcon.textContent = '✅';
      completionLabel.textContent = 'Đã học xong!';
    } else {
      completionBtn.classList.remove('is-done');
      if (completionIcon) completionIcon.textContent = '○';
      completionLabel.textContent = 'Đánh dấu đã học';
    }
  }
  return node;
}

// 4. Patch markCompleted to update button style immediately
const _origMarkCompleted = markCompleted;
function markCompleted(id) {
  _origMarkCompleted(id);
}

// 5. Patch renderAnalysis to use new word-item style with audio btn
...
async function renderAnalysis(root, s) {
  ...
  analysis.forEach(item => {
    const origin = normalizeOrigin(item.origin);
    const cls = origin === 'thuần hàn' ? 'tag-native' : origin === 'hán hàn' ? 'tag-sino' : 'tag-unknown';
    const badge = origin === 'thuần hàn'
      ? '純 Thuần Hàn'
      : origin === 'hán hàn'
        ? `漢 Hán Hàn${item.hanja ? ` (${item.hanja})` : ''}`
        : '? Không rõ';
    ...
    div.innerHTML = `
      <div class="word-head">
        ...
        <button class="word-audio-btn" title="Nghe từ">🔊</button>
      </div>
      <div class="word-meaning">${item.meaning_vi}</div>
      <div class="word-type">${item.type || 'từ vựng'}${item.root && item.root !== item.word ? ` · ${item.root}` : ''}</div>
      <span class="origin-tag ${cls}">${badge}</span>
    `;
    div.querySelector('.word-audio-btn').addEventListener('click', () => speak(item.word, 0.8));
    root.append(div);
  });
}

// 6. Random button active state
document.getElementById('randomBtn').addEventListener('click', () => {
  document.getElementById('randomBtn').classList.toggle('active-random', state.random);
});

// Run streak display on load
